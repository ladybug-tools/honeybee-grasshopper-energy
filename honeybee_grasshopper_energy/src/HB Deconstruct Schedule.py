...
-
    ...
    Args:
        _schedule: A ScheduleRuleSet to be deconstructed into DataCollections
            ...
            be the name of a Schedule to be looked up in the schedule library.
        _timestep_: An integer for the number of steps per hour at which to make
            the resulting daily DataCollections.
    
    Returns:
        day_names: A list of names for each unique ScheduleDay that defines the
            input ScheduleRuleset.
...
if all_required_inputs(ghenv.Component):
    ...
    day_names = [_schedule.default_day_schedule.name]
    day_data = [default]
    for rule in _schedule.schedule_rules:
        ...
        day_names.append(rule.schedule_day.name)
    ...
    if _schedule.summer_designday_schedule is not None:
        ...
        day_names.append(_schedule.summer_designday_schedule.name)
    ...
    if _schedule.winter_designday_schedule is not None:
        ...
        day_names.append(_schedule.winter_designday_schedule.name)
